...
def circuit(cm:Circuit_manager, data_reuploading=False, barrier=False): # keep it outside DQN
        ...
        # def   (inputs, num_qubits): # for cart pole
...
class QDQN(nn.Module):
    ...
    #     return out
    def forward(self, inputs):
        # print(f"isinstance: {isinstance(self.q_layers.qnode, qml.QNode)}, tape = {self.q_layers.qnode.qtape}, wires = {self.q_layers.qnode.device.wires}")
        #print(torch.tensor(qml.matrix(self.q_layers.qnode)(inputs, *self.q_layers.qnode_weights)))

        return qml.matrix(self.q_layers.qnode)(inputs, self.q_layers.theta_weights, self.q_layers.phi_weights, self.q_layers.delta_weights) #TODO:to.(self.device)
